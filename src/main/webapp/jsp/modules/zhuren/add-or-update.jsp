    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑综合部主任</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑综合部主任</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">综合部主任信息</h3>
                            ...
                            <!-- 当前表的字段 -->
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <input id="zhurenPhone" name="zhurenPhone" class="form-control"
                                               onchange="zhurenPhoneChickValue(this)"  placeholder="综合部主任手机号">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>综合部主任身份证号</label>
                                        <input id="zhurenIdNumber" name="zhurenIdNumber" class="form-control"
                                               onchange="zhurenIdNumberChickValue(this)"  placeholder="综合部主任身份证号">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>综合部主任照片</label>
                                        <img id="zhurenPhotoImg" src="" width="100" height="100">
                                        <input name="file" type="file" id="zhurenPhotoupload"
                                               class="form-control-file">
                                        <input name="zhurenPhoto" id="zhurenPhoto-input" type="hidden">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>性别</label>
...
<script>
    ...
    function upload() {
        ...
        <!-- 当前表的文件上传 -->
        $('#zhurenPhotoupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                var photoUrl= baseUrl + 'file/download?fileName=' + data.result.file;
                document.getElementById('zhurenPhotoImg').setAttribute('src',photoUrl);
                document.getElementById('zhurenPhoto-input').setAttribute('value',photoUrl);
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("zhuren/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addzhuren');
    ...
    //数字检查
        <!-- 当前表的数字检查 -->
        function zhurenPhoneChickValue(e){
        ...
        function zhurenIdNumberChickValue(e){
            var this_val = e.value || 0;
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                ...
                password: "required",
                zhurenName: "required",
                zhurenPhone: "required",
                zhurenIdNumber: "required",
                zhurenPhoto: "required",
                sexTypes: "required",
            },
            messages: {
                username: "账户不能为空",
                password: "密码不能为空",
                zhurenName: "综合部主任姓名不能为空",
                zhurenPhone: "综合部主任手机号不能为空",
                zhurenIdNumber: "综合部主任身份证号不能为空",
                zhurenPhoto: "综合部主任照片不能为空",
                sexTypes: "性别不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addzhuren = window.sessionStorage.getItem("addzhuren");
        if (addzhuren != null && addzhuren != "" && addzhuren != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("zhuren/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
