    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑主管领导</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">主管领导管理</li>
                        <li class="breadcrumb-item active">编辑主管领导</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">主管领导信息</h3>
                            ...
                            <!-- 当前表的字段 -->
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <input id="zhuguanPhone" name="zhuguanPhone" class="form-control"
                                               onchange="zhuguanPhoneChickValue(this)"  placeholder="主管领导手机号">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>主管领导身份证号</label>
                                        <input id="zhuguanIdNumber" name="zhuguanIdNumber" class="form-control"
                                               onchange="zhuguanIdNumberChickValue(this)"  placeholder="主管领导身份证号">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>主管领导照片</label>
                                        <img id="zhuguanPhotoImg" src="" width="100" height="100">
                                        <input name="file" type="file" id="zhuguanPhotoupload"
                                               class="form-control-file">
                                        <input name="zhuguanPhoto" id="zhuguanPhoto-input" type="hidden">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>性别</label>
...
<script>
    ...
    function upload() {
        ...
        <!-- 当前表的文件上传 -->
        $('#zhuguanPhotoupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                var photoUrl= baseUrl + 'file/download?fileName=' + data.result.file;
                document.getElementById('zhuguanPhotoImg').setAttribute('src',photoUrl);
                document.getElementById('zhuguanPhoto-input').setAttribute('value',photoUrl);
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("zhuguan/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addzhuguan');
    ...
    //数字检查
        <!-- 当前表的数字检查 -->
        function zhuguanPhoneChickValue(e){
        ...
        function zhuguanIdNumberChickValue(e){
            var this_val = e.value || 0;
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                username: "required",
                password: "required",
                zhuguanName: "required",
                zhuguanPhone: "required",
                zhuguanIdNumber: "required",
                zhuguanPhoto: "required",
                sexTypes: "required",
            },
            messages: {
                username: "账户不能为空",
                password: "密码不能为空",
                zhuguanName: "主管领导姓名不能为空",
                zhuguanPhone: "主管领导手机号不能为空",
                zhuguanIdNumber: "主管领导身份证号不能为空",
                zhuguanPhoto: "主管领导照片不能为空",
                sexTypes: "性别不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addzhuguan = window.sessionStorage.getItem("addzhuguan");
        if (addzhuguan != null && addzhuguan != "" && addzhuguan != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("zhuguan/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
