    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑借阅部领导</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">借阅部领导管理</li>
                        <li class="breadcrumb-item active">编辑借阅部领导</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">借阅部领导信息</h3>
                            ...
                            <!-- 当前表的字段 -->
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <input id="jieyuebuPhone" name="jieyuebuPhone" class="form-control"
                                               onchange="jieyuebuPhoneChickValue(this)"  placeholder="借阅部领导手机号">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>借阅部领导身份证号</label>
                                        <input id="jieyuebuIdNumber" name="jieyuebuIdNumber" class="form-control"
                                               onchange="jieyuebuIdNumberChickValue(this)"  placeholder="借阅部领导身份证号">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>借阅部领导照片</label>
                                        <img id="jieyuebuPhotoImg" src="" width="100" height="100">
                                        <input name="file" type="file" id="jieyuebuPhotoupload"
                                               class="form-control-file">
                                        <input name="jieyuebuPhoto" id="jieyuebuPhoto-input" type="hidden">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>性别</label>
...
<script>
    ...
    function upload() {
        ...
        <!-- 当前表的文件上传 -->
        $('#jieyuebuPhotoupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                var photoUrl= baseUrl + 'file/download?fileName=' + data.result.file;
                document.getElementById('jieyuebuPhotoImg').setAttribute('src',photoUrl);
                document.getElementById('jieyuebuPhoto-input').setAttribute('value',photoUrl);
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("jieyuebu/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addjieyuebu');
    ...
    //数字检查
        <!-- 当前表的数字检查 -->
        function jieyuebuPhoneChickValue(e){
        ...
        function jieyuebuIdNumberChickValue(e){
            var this_val = e.value || 0;
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                username: "required",
                password: "required",
                jieyuebuName: "required",
                jieyuebuPhone: "required",
                jieyuebuIdNumber: "required",
                jieyuebuPhoto: "required",
                sexTypes: "required",
            },
            messages: {
                username: "账户不能为空",
                password: "密码不能为空",
                jieyuebuName: "借阅部领导姓名不能为空",
                jieyuebuPhone: "借阅部领导手机号不能为空",
                jieyuebuIdNumber: "借阅部领导身份证号不能为空",
                jieyuebuPhoto: "借阅部领导照片不能为空",
                sexTypes: "性别不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addjieyuebu = window.sessionStorage.getItem("addjieyuebu");
        if (addjieyuebu != null && addjieyuebu != "" && addjieyuebu != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("jieyuebu/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
